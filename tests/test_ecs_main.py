...
def test_download_static_assets_writes_expected_files(tmp_path, monkeypatch):
    payloads = {
        ...
        "anagram-exclude-words.txt": b"nightmare\n",
    }
    ...
    downloaded = ecs_main.download_static_assets(
        ...
        anagram_exclude_key="anagram-exclude-words.txt",
    )
    ...
    assert Path(downloaded["anagram_exclude"]).read_text(encoding="utf-8") == "nightmare\n"
...
def test_stage_assets_for_generator_copies_to_target_root(tmp_path):
    ...
    (assets_dir / "anagram-exclude-words.txt").write_text("nightmare\n", encoding="utf-8")
    ...
    staged = ecs_main.stage_assets_for_generator(
        {
            ...
            "anagram_exclude": str(assets_dir / "anagram-exclude-words.txt"),
        },
        str(tmp_path / "target"),
    )
    ...
    assert Path(staged["anagram_exclude"]).read_text(encoding="utf-8") == "nightmare\n"
...
def test_main_runs_generation_pipeline_for_target_week(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "anagram_exclude": str(tmp_path / "downloaded-anagram.txt"),
        },
    )
    monkeypatch.setattr(
        ecs_main,
        ...
        lambda downloaded_assets, target_root: {
            ...
            "anagram_exclude": str(tmp_path / "staged-anagram.txt"),
        },
    )
    usage_downloads = []
    usage_uploads = []
...
def test_main_passes_anagram_exclude_file_to_validator_when_staged_file_exists(tmp_path, monkeypatch):
    _write_completed_payloads(tmp_path / "payloads", 7)
    staged_anagram = tmp_path / "staged-anagram.txt"
    staged_anagram.write_text("nightmare\n", encoding="utf-8")
    ...
    monkeypatch.setattr(
        ...
        lambda downloaded_assets, target_root: {
            ...
            "horizontal_exclude": str(tmp_path / "staged-horizontal.txt"),
            "letter_scores": str(tmp_path / "staged-scores.json"),
            "anagram_exclude": str(staged_anagram),
        },
    )
    ...
    def fake_run_generation_pipeline(config, *, cwd=None):
        captured["validator_args"] = config.validator_args
        return (
            subprocess.CompletedProcess(["generator"], 0),
            subprocess.CompletedProcess(["validator"], 0),
        )
    ...
    assert captured["validator_args"] == [
        "--summary-only",
        "--horizontal-exclude-file",
        str(tmp_path / "staged-horizontal.txt"),
        "--anagram-exclude-file",
        str(staged_anagram),
    ]
