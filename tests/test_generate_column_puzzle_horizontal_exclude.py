...
def test_choose_rows_respects_horizontal_exclude_set():
    ...
    # Validate row constraints were still honored.
    for r, w in enumerate(rows):
        assert w[0] == "abcde"[r]
        assert w[r] == "abcde"[r]


def test_forced_targets_can_still_use_excluded_words():
    mod = _load_generator_module()
    ...
    # Excluded words are still valid as VT/DT targets.
    assert target == "abcde"
    assert diag == "abcde"
    assert col == 0
    assert ddir == "main"
    # But excluded words should not appear as horizontal rows.
    assert all(w not in excluded for w in rows)
